...
class CMIPDownloader:
    ...
    def setup_connection(self):
        ...
        def condition_1(result):
            if ("datetime_start" in list(result.json.keys())) and (
                "datetime_stop" in list(result.json.keys())
            ):
                return True
            else:
                return False
        ...
        def condition_2(result):
            if result.json["experiment_id"][0] in ["ssp126", "ssp585"]:
                if (string_to_date(result.json["datetime_start"]).year != 2015) or (
                    string_to_date(result.json["datetime_stop"]).year < 2099
                ):
                    return False
                else:
                    return True
    ...
    def filter_models(self):
        ...
        for model in dic_filt:
            ...
            for variant in list(dic_filt[model].keys()):
                ...
                if sorted(list(dic_filt[model][variant].keys())) != sorted(
                    self.variables
                ):
                ...
                for variable in list(dic_filt[model][variant].keys()):
                    if sorted(dic_filt[model][variant][variable]) != sorted(
                        self.experiments
                    ):
                        del dic_filt[model][variant]
                        skip_variable_loop = True
                        break
                    dic_filt[model][variant][variable] = sorted(
                        dic_filt[model][variant][variable]
                    )
    ...
    def create_dataframe(self):
        ...
        def get_matching_results(model, variable, experiment):
            return [
                result
                for result in self.results
                if (result.json["source_id"][0] == model)
                and (result.json["variable"][0] == variable)
                and (result.json["experiment_id"][0] == experiment)
                and ("datetime_start" in result.json.keys())
                and ("datetime_stop" in result.json.keys())
            ]
        ...
        for model in models_dict:
            ...
            for variable in self.variables:
                nominal_resolution = [
                    result.json["nominal_resolution"]
                    for result in self.results
                    if (result.json["source_id"][0] == model)
                    and (result.json["variable"][0] == variable)
                ][0][0]
                ...
                for variable in self.models_dict_filtered[model][variant]:
                    for experiment in self.models_dict_filtered[model][variant][
                        ...
                        results_match = [
                            ...
                            for result in self.results
                            if (result.json["source_id"][0] == model)
                            and (result.json["variant_label"][0] == variant)
                            and (result.json["variable"][0] == variable)
                            and (result.json["experiment_id"][0] == experiment)
                        ]
                        ...
                        for _result in results_match:
                            ...
                            if experiment in ["ssp126", "ssp585"]:
                                files = [
                                    {"filename": f.filename, "url": f.download_url}
                                    for f in hit
                                    if int(f.filename.split("_")[-1][:4]) >= 2015
                                    and int(f.filename.split("_")[-1][id_i:id_f])
                                    <= 2150
                                ]
                            else:
                                files = [
                            ...
                            if len(files) > 0:
                                ...
                                for index, row in files.iterrows():
                                    ...
                                    path_crop = path.replace('.nc', f'_cropped.nc')
                                    #path_crop = ofo + f"{experiment}_cropped.nc"
                                    if os.path.isfile(path) or os.path.isfile(
                                        path_crop
                                    ):
                                        ...
                                        if crop_region is not None:
                                            ...
                                            os.remove(path)
                                # if crop_region is not None:
                                #     if not (os.path.isfile(path_crop)):
                                #         print("Cropping...")
                                #         ds = xr.open_mfdataset(
                                #             ofo + "*.nc", combine="by_coords"
                                #         )
                                #         ds_crop = utils.crop_by_polygon(
                                #             ds, polygon=crop_region
                                #         )
                                #         ds_crop.to_netcdf(path_crop)
                                #         for index, row in files.iterrows():
                                #             path = ofo + row.filename
